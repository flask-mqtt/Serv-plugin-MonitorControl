...
try:
    ...
    print('初次安装环境pythoncom异常捕获，执行重启(正常请无视这条报告，仅是一个提醒)。')
    import os
    os.system('run')
    import sys
    try:
            ...
            for monitor in wmi.WmiMonitorDescriptorMethods():
                try:
                    ...
                    try:
                        ...
                        try:
                            man_id, manufacturer = _monitor_brand_lookup(
                                manufacturer)
                        except TypeError:
...
class VCP:
    ...
    @classmethod
    def get_VCP(cls, code:int, display: Optional[int] = None, max_tries: int = 50) -> List[int]:
        '''
            根据VCP代码获取结果
        '''
        code = BYTE(code)
    ...
    @classmethod
    def set_VCP(cls, code: int, value: int, display: Optional[int] = None, max_tries: int = 50):
        '''
            设置VCP代码
        '''
...
def list_monitors_info(method: Optional[str] = None, allow_duplicates: bool = False) -> List[dict]:
    ...
    Args:
        method (str): the method the monitor can be addressed by. See `VCP_control.get_methods`
